...
export function getValueType(value, options = {}) {
  ...
  // Local addition: testing "YYYY-MM-DDTHH:mm:ss.SSSZ" date format.
  // Safe to test last, because d3 only returns a date when the pattern
  // consumes the whole string, so the formats above cannot match this one.
  if (dateParser) {
    const dateFormatTest = dateFormats["YYYY-MM-DDTHH:mm:ss.SSSZ"]
    const testDateWithFormat = dateParser(dateFormatTest)(value)
    if (testDateWithFormat !== null) {
      return {
        type: "date",
