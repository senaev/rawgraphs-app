...
export class LocaleNumberParser {
  constructor(locale) {
    ...
    // Local fix: upstream had `group ||` here, but this constructor takes only
    // `locale`, so `group` was an undefined reference. The sibling NumberParser
    // does destructure `group`; this class was copied from it. Matches the
    // referenced notebook, which has no such fallback.
